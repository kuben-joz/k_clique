...
__device__ unsigned int neigh_bitmasks[g_const::max_deg][32];
__device__ unsigned int lvl_bitmap[g_const::threads_per_block / tile_size][g_const::max_clique_size - 4][32]; // todo this might be +1, will be buggy regardless if I set max_clqiue_size ot 3 during testing

__device__ int countSubtreeCliques(int level, int *num_cliques, int clique_size,
                                   int neigh_ids[g_const::max_deg], int neigh_bitmasks[g_const::max_deg][32],
                                   int v_idx, int v_neigh_idx, int *row_ptrs, int *v1s, int *v2s)
{
}

const int tile_sz = tile_size;
// todo change to template, if I forgot I had to change for intellisensen to work during dev, sorry
__device__ void calculateIntersects(int v_idx, int *row_ptrs, int *v1s, int *v2s, int clique_size)
{
    ...
    for (int ii = block.thread_rank(); ii < num_neighs; ii += blockDim.x)
    {
    ...
    int lvl_idx[g_const::max_clique_size - 1];                 // todo this mioght be +1
    int lvl_num_neighs[g_const::max_clique_size - 2];          // todo this might be + 1, also might be unneccesary
        ...
        do
        {
            while (lvl_idx[current_level] < num_neighs)
            {
                ...
                if (idx == 0)
                {
                    ...
                    cg::reduce(tile, lvl_num_neighs[current_level], cg::plus<int>());
                    if (lvl_num_neighs[current_level] > 0 && tile.thread_rank() == 0)
                    {
                        num_cliques_total[current_level + 2].fetch_add(lvl_num_neighs[current_level], cuda::memory_order_relaxed); // todo make the wraparound work wiht unsigned ints by adding unsigned int max - 10^9 every time
                    }
...
__global__ void countCliquesKern(int *row_ptrs, int *v1s, int *v2s, int *res, int clique_size)
{
    ...
    __shared__ int block_res;
    for (int ii = blockIdx.x; ii < g_const::num_vertices_dev; ii += blockDim.x * gridDim.x)
    {
        calculateIntersects(ii, row_ptrs, v1s, v2s, clique_size);
    }
}

void countCliques(Graph g, int clique_size)
{
    int *res_dev;
    cudaMalloc((void **)&res_dev, sizeof(int) * clique_size);
